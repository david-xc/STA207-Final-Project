...
Within these variables, we want to answer three questions:
...
Note that we only consider the variables mentioned in last paragraph.
...
write.csv(bb_total,"Data/Totaldata.csv", row.names = FALSE)
...
Age_65up <- read.csv("Data/Age_65up.csv",
               header = FALSE)
...
Age_1564 <- read.csv("Data/Age_15_64.csv",
               header = FALSE)
...
Age_under14 <- read.csv("Data/Age_under14.csv",
               header = FALSE)
...
```{r,echo=F,results=T,message=F}
world <- map_data("world");
stringency1 <- read.csv("Data/stringency-index1.csv",
               header = FALSE)
...
Summary_data <- read_csv("Data/Summary data.csv")
...
X represents the vector that contain all the variables and its observations. $\beta$ represent the regression coefficient vector.And ϵ is the scalar residual term. And the goal of our model is to subset a best model from the full model that can indicate which variables does most effects to our response variables. And we will select the best subset model form the full model by the following criterion: adjusted R square (Bigger is better), AIC and BIC(stands for Akaike's Information Criteria and Bayesian Information Criteria, lower is better)

The reasons to use the linear regression model is because this model is reliable when analyzing few variables, since we only have 10 variables here and we will remove most of them in the subset procedure. Also linear regression model is easy to analyze and easy to construct which improve the efficiency of our project. 

Also after we got the subset model we will double check the result by the stepwise procedure, even though it is an old and primitive methods, the accuracy of such methods is well guaranteed while doing selection on the linear regression model. And most importantly this method is easily to access and being used for many years which proved its reliability. 
...
By the stepwise procedure we have the same result as we have before in the subset procedure. Model 4 will be our best model. Which conclude that the variables Age_15_64, Age_0_14, Temperature and GDS fit the linear regression model best. Thus we could say that the population age between 15-64, population age between 0-14, average Temperature and the Gross domestic savings have the significant association with our Cumulative Cases. Also for our model, the regression coefficients vector is $[ \beta_0=6.152822e+05~  \beta_1=5.067040e-06~  \beta_2=1.833155e-01~  \beta_3=-7.090291e-02~  \beta_4=-3.252580e+04]$

Continue with the Cumulative death, we can construct another similar model for it. 
...
Finally we can analyze on the term $\frac{Cumulative~cases}{Cumulative~deaths}$
...
```
Finally we could say that the Stringencyindex, popdensity, tracing,  and Age_65up have the significant association with ratio between Cumulative deaths and Cumulative cases.  Also for our model, the regression coefficients vector is $[ \beta_0= 1.616851e-02~  \beta_1=-2.954249e-06 2~  \beta_2=-4.992233e-03~  \beta_3=1.364362e-04~  \beta_4=1.315491e-10]$

And sometimes we will get the over fit coefficient in the model, and the reason is because the observation value for those variables is quite big compare to other variables, such as for the GDS, the average value of the GDS is about 160 billion which is way bigger than other variables. 

### Causal Interpretation of the propesed model.
When we got those subsets model, we find that only model for the case-mortality rate have the binary variable tracing. And we want to check whether it is true by the casual inference, first we have checked the propensity score of this model.
...
Conduct the hypothesis test, we have $H_0$:The coefficient of the variables tracing is 0. $H_1$:The coefficient of the variables tracing is not 0.
...
And the test result suggest that we need to reject the null hypothesis. Then we have the conclusion that the tracing variable is associated with the case-mortality rate. And the possible reasons behind the conclusion might be those, in general the people are infected will pay more attention to all kinds of information regard to the pandemic, including the trace of other patient, by the trace information, they will avoid meeting with other patients or heavily loaded hospital. Which has raise their survival rate in somehow.  
...
In the Normal Q-Q plot, the shape is very strange and many points deviates from the line a lot. We used Shapiro-Wilk Normality Test on the residual to check the normality.

```{r,echo=F}
shapiro.test(step.f1$residuals)
```

The p-value is less than 2.2e-16, which implies that the distribution of the residuals are significantly different from normal distribution.
...
The shape of the Q-Q plot is very strange. The normality assumption might be violated. We used Shapiro-Wilk Normality Test to check the normality.

```{r,echo=F}
shapiro.test(step.f2$residuals)
```
...
In this Normal Q-Q plot, there are some point deviates from the dash line, but most of the points are on the lines. We used Shapiro-Wilk Normality Test to check the normality.

```{r,echo=F}
shapiro.test(step.f3$residuals)
```

The p-value is less than 3.898e-08, which implies that the distribution of the residuals are significantly different from normal distribution.

```{r,echo=F}
Summary_data[c(19, 28, 37, 57, 85, 95, 119, 133), 1]
```

These countries are the interest data points in the diagnostic plots. United States of America (#133) appears 6 times, and Brazil (#19) and China (#28) appear 4 times each. These countries have very harge national territorial and very complex, which make some predictive variables, such as average temperature, population density and stringency index, less representative. 
...
It important to notes that the normality assumption in our models do not hold. Fortunately, this violation does not affect our project too much, since the normality assumption is important when we need to get confidence intervals and p-value from our analysis.
...
Github repository: https://github.com/david-xc/STA207-Final-Project/blob/main/Sta207_Project3.Rmd
```{r}
sessionInfo()
```
